...
description: 接收已建立的 plan 文件（單一、多個、或資料夾），從 3-Progressing 開始連續推進到 8-Archived，並完成 summary。
version: 2.1.0
...
> **未收到明確確認前，絕對不可自行推進任何後續搬移或推進動作。這條規則優先於一切其他規則。**
>
...
- **【強制停頓】以上驗證全部通過後，必須先向使用者回報驗證結果與來源清單，確認完成後才可開始任何搬移或推進動作。未收到使用者確認前，絕對不可執行任何搬移。**
