...
5. 測試通過只能代表既有測試案例全綠，**不得視為需求已完整完成的充分證明**。
6. 必須逐條比對需求文件、Spec、Plan、驗收條件、使用者描述、實際程式碼、測試內容與執行結果，找出所有遺漏、行為不一致、邊界情境不足、文件不同步與可維護性風險。
7. 所有查證出的缺失、建議、需要補強的地方，以及後續實際修復或修改的內容，都必須完整詳細記錄到任務文件與歸檔 summary 中。
8. 執行 verify 時，除了檢查是否符合原始需求，也必須主動針對內容修改提出任何可讓需求、文件、測試或程式碼更完整的建議，並告知使用者。

## 🔴 全面需求符合度驗證鐵律（測試通過不等於任務完成）
...
1. **需求內容完整性**：需求描述、驗收條件、邊界情境、錯誤流程、權限與安全性要求是否需要補充。
2. **程式碼修改完整性**：實作是否有可讀性、可維護性、命名、錯誤處理、重複邏輯、資料驗證或相依性管理上的改善建議。
3. **測試內容完整性**：是否需要新增正向流程、錯誤流程、邊界條件、權限情境、資料格式錯誤或整合測試。
4. **文件內容完整性**：README、API 文件、Spec、Plan、任務文件與實際行為是否一致，是否需要補充使用方式、限制、注意事項或範例。
5. **交付內容完整性**：是否有遺漏設定檔、環境變數說明、遷移步驟、部署注意事項、相容性說明或後續追蹤事項。
...
> **強制執行至少三次的獨立檢查迴圈，每次檢查必須著重不同面向（例如：第一次逐條核對需求與程式碼、第二次核對文件、註解與測試覆蓋範圍、第三次執行測試並回頭確認測試是否真的覆蓋需求）。**
> **只要在任一次檢查中發現需求遺漏、程式碼行為不符、測試不足、文件不同步、風險建議或測試失敗，必須立即將問題條列出來，寫入任務文件的 Verify 查核紀錄，並詢問使用者是否開始解決這些問題。如果使用者同意，則開始處理，處理完成後，自動開始跑下一輪檢查，一直反覆到所有輪數都完成，且沒有任何問題。**
...
- 確認要檢查的來源路徑。
- 讀取其對應的原始需求、Spec、Plan、README、API 文件、使用者補充說明與驗收條件。
- 將所有需求拆成可逐條驗證的需求清單，不得只用整體摘要帶過。
- 收集所有在本次任務中被修改或建立的原始碼、測試檔、設定檔、遷移檔、文件與任務文件清單。
- 建立「需求追蹤矩陣」，把每一條需求對應到實際程式碼檔案、測試檔案與驗證方式。
- 若找不到某條需求的對應程式碼或測試，必須立即標記為缺失或需要補強，不得因為既有測試通過就忽略。

2. 第一回合：需求與規格比對檢查 (Round 1: Requirement Alignment)

- 從需求文件出發，逐條追蹤到實際程式碼邏輯、資料流、錯誤處理、權限檢查、邊界條件與使用者可見行為。
- 將修改後的程式碼邏輯與 Spec/Plan 上的驗收標準 (Acceptance Criteria) 逐一比對。
- 確認所有列出的需求都已被滿足，沒有遺漏功能，也沒有多做不在範圍內的功能。
- 檢查是否存在「測試有寫到，但需求沒有要求」或「需求有要求，但測試與程式碼沒有處理」的落差。
- 將每一項比對結果寫入「需求追蹤矩陣」，並清楚標示 ✅ 通過、❌ 缺失、⚠️ 需要補強。
- 若發現缺失或需要補強，必須具體記錄：需求來源、缺失描述、影響範圍、建議修法、預計修改檔案。
- 即使需求已符合，也必須檢查是否有讓內容更完整、規格更清楚、錯誤情境更明確或實作更健壯的建議，並告知使用者。

3. 第二回合：文件與測試範圍檢查 (Round 2: Doc & Test Coverage)

- 檢查關聯任務文件狀態及勾選項目是否已詳實填寫。
- 檢查測試檔 (`.test.ts`, `.spec.ts` 等) 內容：確認其涵蓋了正向路徑 (Happy path)、預期的錯誤情境 (Edge cases)、權限或安全性情境、資料為空或格式錯誤情境，以及需求中特別指定的例外流程。
- 檢查測試名稱、測試資料與 assertion 是否真的驗證需求行為，而不是只驗證函式有被呼叫或 snapshot 沒變。
- 檢查測試是否有過度 mock 導致未驗證真實行為；若有，必須列為補強建議。
- 確認修改到的功能如有架構異動，對應的 README 或教學文件是否有同步更新。
- 確認任務文件已記錄所有查證出的缺失、建議、需要補強之處，以及後續實際修復或修改內容。
- 若 README、API 文件、Spec、Plan 與實際程式碼行為不一致，必須列為缺失並修正或明確記錄為待使用者決策。
- 主動檢查所有文件內容是否還有可補充更完整的地方，例如限制條件、使用範例、環境變數、部署注意事項、測試方式、錯誤排查方式與後續維護建議。
...
- 若沒有發生任何錯誤，在相關任務文件內補上測試通過的截錄記錄、需求再比對結果、未發現缺失的明確理由。
- 若修復過任何缺失，必須再次執行受影響測試，並把「修復前缺失 → 修改內容 → 修復後驗證結果」完整記錄到任務文件。
- 測試全綠後仍必須輸出「內容修改建議審查」結果；若沒有任何建議，也必須明確寫出「未發現需補強建議」及判斷依據，不得省略此步驟。
...
- 條列這幾次檢查中所確認的重點（例：已逐條檢查 8 項需求、已確認 3 個測試檔涵蓋範圍、已執行測試共 15 個 test cases 皆通過）。
- 報告中必須列出需求追蹤矩陣摘要、查證出的缺失清單、補強建議清單、實際修復或修改內容清單，以及修復後再驗證結果。
- 報告中必須列出「內容修改建議審查」摘要，包含必須修復、強烈建議補強、可選改善三類項目；若沒有建議，必須列出明確判斷依據。
...
- 每一輪的檢查中，如果發現有問題、缺失、bug、需求未覆蓋、測試不足、文件不同步、可維護性風險、內容可補充更完整之處或任何修改建議，立即列出問題與建議清單，寫入任務文件的 Verify 查核紀錄，並詢問使用者是否開始解決這些問題或採納建議。
- 若使用者同意，則自動開始修復程式碼、測試或文件。修復完成後，必須把實際修復或修改內容寫入任務文件，再自動開始跑下一輪檢查（重新跑流程確保無遺漏），反覆此流程直到所有輪數完成且零缺失。
- 若使用者暫時不處理缺失或建議，仍必須把缺失、建議、影響範圍、建議處理方式與未處理原因記錄到任務文件。若屬「必須修復」類別，不得宣稱 verify 通過。
...
- **絕對不得**把測試通過視為需求已完整完成的充分證明。測試全綠後仍必須逐條比對需求文件、Spec、Plan、程式碼、測試與文件。
- **絕對不得**只檢查測試檔是否通過，而未檢查實際程式碼是否完整符合需求、是否有遺漏需求、是否需要補強、是否有文件不同步。
- **絕對不得**省略需求追蹤矩陣。每一條需求都必須有對應程式碼、測試或明確缺失紀錄。
- **絕對不得**發現缺失或建議後只在對話中說明而不寫入任務文件或 archived summary。
- **絕對不得**在 verify 時只回報錯誤，不主動提出內容可以補充更完整、修改更清楚、測試更完善或文件更一致的建議。
- **絕對不得**因為需求已通過就省略「內容修改建議審查」。即使沒有建議，也必須記錄「未發現需補強建議」與判斷依據。
- **絕對不得**把「可選改善」與「必須修復」混在一起。必須清楚分類，避免使用者誤以為所有建議都阻擋 verify，或誤以為阻擋項目只是可選改善。
- **絕對不得**完成修復後只說「已修復」。必須記錄實際修改檔案、修改內容、修復原因、修復後測試指令與需求再比對結果。
