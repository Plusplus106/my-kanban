...
5. **為每個計畫評估複雜度和依賴關係**
   - ⛓️ **spec 層前置檢查（警告不阻擋）：** 拆解前先看該 spec 資料夾內是否有 `[spec-xxxxx]-RELATIONS.md`；若「前置 Spec」尚未全數在 `7-Done/8-Archived`，必須向使用者提示「前置 [spec-xxxxx] 尚未完成，plan 細節可能過時」，由使用者當場決定是否繼續拆解（詳見 `templates/0-Epics/EPICS_RULES.md` 依賴閘門語意）
6. **提供並行執行建議**
   - 明確標示哪些計畫可以並行執行
