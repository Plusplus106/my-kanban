...
> - 🚫 **最高優先級：嚴禁擅自執行完整測試（full test / 全套件 / coverage）** — 預設只跑與本次修改相關的 targeted 測試；要跑完整測試**必須先問使用者並得到明確同意**
...
1. **任務必須放在日期-spec 編號-feature 名稱資料夾中**
   - 例如: `4-Testing/2025-12-01-[spec-Xm1q2]-payment-integration/2025-12-01-[spec-Xm1q2]-2-high-[plan-Az4a2]-feature-stripe-integration.md`
2. **從其他階段移動任務時，保持相同的日期和 spec 資料夾結構**
3. **⚠️ 資料夾命名格式:** `[YYYY-MM-DD]-[spec-xxxxx]-[feature-name]`
   - ❌ 錯誤: `4-Testing/2025-12-01-[spec-Xm1q2]/`（缺少 feature-name）
   - ❌ 錯誤: `4-Testing/2025-12-01-[plan-Xm1q2]-payment-integration/`（不可使用 `[plan-xxxxx]` 作為父資料夾識別）
   - ✅ 正確: `4-Testing/2025-12-01-[spec-Xm1q2]-payment-integration/`
...
1. 建立或補齊測試程式碼。
2. 執行與本次修改相關的 targeted 測試範圍（涵蓋所有異動檔及其直接影響的測試）。**若判斷需要執行完整測試範圍，必須先詢問使用者並取得明確同意，不得自行決定執行。**
