...
- **里程碑表也必須同步更新**：所有里程碑狀態不得殘留「📋 待開始」「TBD」等規劃期值，必須反映實際完成狀態與日期。
...
同時逐卡檢查必填欄位皆有實質內容，**不得殘留佔位符**（例如「本階段完成摘要: -」）；「延後到批次／他卡執行」的子任務，完成後必須回頭補勾或標註「不適用」與理由，不得留空造成卡內自相矛盾。
...
- [ ] 完成標記皆為 `[✓]`
- [ ] `0-PLAN_OVERVIEW` 統計、任務狀態與**里程碑表**皆為最終狀態（無「📋 待開始」「TBD」殘留）
- [ ] 每份 done 任務必填欄位皆有實質內容，無佔位符（如「本階段完成摘要: -」）；延後執行的子任務已回頭補勾或標註「不適用」
...
**版本:** 1.3
**最後更新:** 2026-07-18（增列 overview 里程碑與必填欄位無佔位符檢查；修正檢查清單重複項）
