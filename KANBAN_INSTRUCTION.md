...
> 🚫 **測試範圍鐵律：預設只跑與本次修改相關的 targeted 測試。**
> 完整測試（full test / 全套件 / coverage）**一律必須先詢問使用者並取得明確同意**才可執行。
> 本閘門的任何條文都不得被解讀成「授權自行執行完整測試」。

只要任務文件進入 `4-Testing/`，必須同時滿足以下條件，才可推進：

1. 已建立或補齊測試程式碼（不可只移動文件）。
2. 已執行與本次修改相關的 targeted 測試範圍（涵蓋所有異動檔及其直接影響的測試）。**若判斷需要執行完整測試範圍，必須先詢問使用者並取得明確同意，不得自行決定執行。**
3. 全部測試通過（all green）。
